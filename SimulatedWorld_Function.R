...
SimulateWorld <- function(temp_diff, temp_spatial, PA_shape, abund_enviro){
  ...
  for (y in years){
    ...
    if(temp_spatial == "matern") {
      # JS: *** this appears to flip orientation of temp map generated by "simple"
    ...
    if (PA_shape == "logistic") {
      #specifies alpha and beta of logistic - creates almost knife-edge absence -> presence
    ...
    if (PA_shape == "logistic_prev") {
      ...
      suitability_PA <- virtualspecies::convertToPA(envirosuitability$suitab.raster, PA.method = "probability", beta = "random",
    ...
    if (PA_shape == "linear") {
      ...
      suitability_PA <- virtualspecies::convertToPA(envirosuitability$suitab.raster, PA.method = "probability",
  ...
  if (abund_enviro == "lnorm_low") {
    # JS: *** this doesn't create a new rlnorm value for each cell or year - do you want it to?
    # SB's original
  ...
  if (abund_enviro == "poisson") {
    ...
    for (rr in 1:nrow(grid)) {
      if(grid$pres[rr] == 1) {
        grid$abundance[rr] <- rpois(1, lambda=grid$suitability[rr]*maxN)
      } else { grid$abundance[rr] <- 0 }
    }
  }
